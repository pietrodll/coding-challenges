...
func TestParseInput(t *testing.T) {
	...
	assert.Equal(t, expectedBoard, boards[0])
}

func TestPlayGame(t *testing.T) {
	winnerScore := playGame(parseInput(input))

	assert.Equal(t, 4512, winnerScore)
}

func TestPlayGameUntilLastWins(t *testing.T) {
	lastToWinScore := playGameUntilLastWins(parseInput(input))

	assert.Equal(t, 1924, lastToWinScore)
}
